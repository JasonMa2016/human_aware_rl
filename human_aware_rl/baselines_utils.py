...
# def get_config_from_pbt_dir(pbt_dir):
#     run_path = "data/pbt_runs/{}/seed_0".format(pbt_dir)
#     config = load_dict_from_file(run_path + "/config")
#     return config

# def setup_mdp_env(mdp_params, env_params, mdp_fn_params={}, display=False): 
#     # TODO: Address the fact that now we have a bunch of different functions 'from_config'
#     # that should be merged. Possibly replace env initializer with this
#     """
#     Sets up a function that returns a new instance of the desired mdp, 
#     whether it is fixed (and thus it performs a reset), or variable 
#     (and thus creates a new mdp instead of resetting).

#     Needs keys: 
#     "ORDER_GOAL", "EXPLOSION_TIME", "REW_SHAPING_PARAMS", "ENV_HORIZON"
#     if "FIXED_MDP" == str:
#         "RND_POS", "RND_OBJS"
#     if list:
#         "PADDED_MDP_SHAPE"
#     if false:
#         "PADDED_MDP_SHAPE", "MDP_SHAPE_FN", "PROP_EMPTY_FN", "PROP_FEATS_FN"
#     """
#     if mdp_fn_params == {}:
#         print(mdp_params)
#         mdp = OvercookedGridworld.from_layout_name(**mdp_params)
#         return OvercookedEnv(mdp, **env_params)

#     elif "mdp_choices" in mdp_fn_params.keys() and mdp_fn_params["mdp_choices"] is not None:
#         # TODO: update this
#         # If list of MDPs, randomly choose one at each reset
#         def mdp_generator_fn():
#             chosen_mdp = np.random.choice(mdp_fn_params["mdp_choices"])
#             mdp = OvercookedGridworld.from_layout_name(chosen_mdp, **mdp_params)
#             lg = LayoutGenerator(mdp_fn_params["PADDED_MDP_SHAPE"], mdp_params["ORDER_GOAL"], mdp_params["REW_SHAPING_PARAMS"])
#             mdp_padded = lg.padded_mdp(mdp)
#             return mdp_padded
#         return OvercookedEnv(mdp_generator_fn, **env_params)

#     else:
#         layout_generator = LayoutGenerator(mdp_fn_params["PADDED_MDP_SHAPE"], mdp_params["ORDER_GOAL"], mdp_params["REW_SHAPING_PARAMS"])
#         mdp_generator_fn = lambda: layout_generator.make_disjoint_sets_layout(
#             inner_shape=[rnd_int_uniform(*dim) for dim in mdp_fn_params["MDP_SHAPE_FN"]],
#             prop_empty=rnd_uniform(*mdp_fn_params["PROP_EMPTY_FN"]),
#             prop_features=rnd_uniform(*mdp_fn_params["PROP_FEATS_FN"]),
#             display=display
#         )
#         return OvercookedEnv(mdp_generator_fn, **env_params)
...
# # TODO: Figure out what to do with this use_pretrained_weights
# if use_pretrained_weights:
#     with tf.Session() as sess:
#         sess.run(tf.global_variables_initializer())
#         if len(tf.trainable_variables("behavior_cloning")) == 0: # need to make sure this is correct
#             saver = tf.train.import_meta_graph('./bc/model.ckpt.meta', import_scope="behavior_cloning")
#             saver.restore(sess, tf.train.latest_checkpoint('./bc'))

#         model_from_vars = tf.trainable_variables("behavior_cloning")
#         model_to_vars = tf.trainable_variables(model.scope + "ppo2_model/pi")
#         overwrite_variables(model_from_vars, model_to_vars)