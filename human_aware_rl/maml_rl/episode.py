...
class BatchEpisodes(object):
    def __init__(self, batch_size, gamma=0.95, device='cpu'):
        ...
        self._new_observations_list = [[] for _ in range(batch_size)]
        self._successes_list = [[] for _ in range(batch_size)]
        self._mask_list = []

        self._observations = None
        self._actions = None
        self._rewards = None
        self._new_observations = None
        self._returns = None
    ...
    def new_observations(self):
        if self._new_observations is None:
            observation_shape = self._new_observations_list[0][0].shape
            observations = np.zeros((len(self), self.batch_size)
                + observation_shape, dtype=np.float32)
            for i in range(self.batch_size):
                length = len(self._new_observations_list[i])
                observations[:length, i] = np.stack(self._new_observations_list[i], axis=0)
            self._new_observations = torch.from_numpy(observations).to(self.device)
        return self._new_observations
    ...
    def successes(self):
        if self._successes is None:
            successes = np.zeros((len(self), self.batch_size), dtype=np.float32)
            for i in range(self.batch_size):
                length = len(self._successes_list[i])
                successes[:length, i] = np.stack(self._successes_list[i], axis=0)
            self._successes = torch.from_numpy(successes).to(self.device)
    ...
    def append(self, observations, actions, rewards, new_observations, batch_ids):
        for observation, action, reward, new_observation, batch_id in zip(
                observations, actions, rewards, new_observations, batch_ids):
            if batch_id is None:
                continue
            ...
            self._new_observations_list[batch_id].append(new_observation.astype(np.float32))
            # self._successes_list[batch_id].append(success.astype(np.float32))
    def __len__(self):
